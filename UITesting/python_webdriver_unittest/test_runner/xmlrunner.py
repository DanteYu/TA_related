...
class _TestInfo(object):
    ...
    def __init__(self, test, time_val):
    ...
    @staticmethod
    def create_success(test, time_val):
        """Create a _TestInfo instance for a successful test."""
        return _TestInfo(test, time_val)

    @staticmethod
    def create_failure(test, time_val, failure):
        """Create a _TestInfo instance for a failed test."""
        info = _TestInfo(test, time_val)
        info._failure = failure
        return info

    @staticmethod
    def create_error(test, time_val, error):
    ...
    def _print_error(self, stream, tagname, error):
        ...
        stream.write('    <%s type="%s">%s\n' \
            % (tagname, str(error[0]).replace("<type '", "").replace("'>", ""), text))
        tb_stream = StringIO()
